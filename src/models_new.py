...
@dataclass
class Stage1Result:
    """Results from Stage 1 processing: File enumeration and metadata collection."""
...
@dataclass
class Stage2Result:
    """Results from Stage 2 processing: AI model selection and preparation."""
    ...
    def get_model_for_file(self, file_info: FileInfo) -> str:
        """
        Get the recommended model for a specific file.
        ...
        Returns:
            Model name string
        """
        return self.mime_to_model_mapping.get(file_info.mime_type, "unknown")

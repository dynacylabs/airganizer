#!/usr/bin/env python3
"""
Script to convert all video files to a uniform format (MP4 with H.264).
...
def convert_video(input_path, output_path, quality='23', preset='medium', verbose=False):
    """
    ...
    Args:
        ...
        quality: CRF value (0-51, lower=better, 23=default, 18=high quality)
    ...
    try:
        ...
        cmd = [
            ...
            '-crf', str(quality),  # Quality (23 = good default)
        ...
        return False, "Conversion timeout (1 hour)"
    except Exception as e:
        return False, str(e)


def scan_and_convert_video(directory_path, quality='23', preset='medium', delete_original=False, dry_run=True, verbose=False):
    """
    Scan directory and convert all video files to MP4.
    """
    if not check_ffmpeg():
    ...
    print(f"Target format: MP4 (H.264 video + AAC audio)")
    print(f"Quality: CRF {quality}, Preset: {preset}")
    if dry_run:
    ...
    for file_path in tqdm(all_files, desc="Checking MIME types", unit="file"):
        ...
        if mime_type and mime_type.startswith('video/'):
            video_files.append((file_path, mime_type))
    ...
    for file_path, mime_type in video_files:
        if mime_type == 'video/mp4' and file_path.suffix.lower() == '.mp4':
            already_mp4.append((file_path, mime_type))
        else:
            to_convert.append((file_path, mime_type))
    
    print(f"\nAlready MP4: {len(already_mp4)} files (will skip)")
    print(f"To convert: {len(to_convert)} files")
    
    if not to_convert:
        print("\nℹ️  All video files are already in MP4 format!")
        return
    ...
    for file_path, mime_type in progress_bar:
        ...
        if output_path == file_path:
            skipped.append((file_path, "already .mp4"))
            continue
        
        # Convert
        success, error = convert_video(file_path, output_path, quality, preset, verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert all video files to MP4 format',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
This script converts all video files to MP4 format using ffmpeg.

Target format: MP4 (H.264 video + AAC audio)
Default quality: CRF 23 (good balance)
Metadata: All tags are preserved (title, description, etc.)

Quality (CRF) guide:
  - 18: High quality (visually lossless, large files)
  - 23: Default (good quality, recommended)
...
Supported input formats:
  - AVI, MOV, WMV, FLV, MKV, WebM, MPEG, 3GP
  - And any other format ffmpeg supports

Examples:
  # Dry run (default) - see what would be converted
  python convert_video.py /path/to/directory
  
  # Actually convert files
  python convert_video.py /path/to/directory --convert
  
  # Convert and delete originals
  python convert_video.py /path/to/directory --convert --delete-original
  
  # Higher quality (CRF 18, slower preset)
  python convert_video.py /path/to/directory --convert --quality 18 --preset slow
  
  # Faster encoding (larger files)
  python convert_video.py /path/to/directory --convert --preset fast
  
  # Show detailed output
  python convert_video.py /path/to/directory --convert --verbose
        '''
    )
    ...
    parser.add_argument('--quality', default='23',
    ...
    if args.convert:
        print(f"\n⚠️  WARNING: This will convert all video files to MP4 (H.264 + AAC)!")
        print(f"   Directory: {args.directory}")
        print(f"   Quality: CRF {args.quality}, Preset: {args.preset}")
